...
public class UIUtilities {
    ...
    ///Creates a pose overlay view for WarriorU Specifically
    public static func createPoseOverlayViewWarriorU(
        forPose pose: Pose, inViewWithBounds bounds: CGRect, lineWidth: CGFloat, dotRadius: CGFloat,
        positionTransformationClosure: (VisionPoint) -> CGPoint, color: UIColor
    ) -> UIView {
        let overlayView = UIView(frame: bounds)
        
        for (startLandmarkType, endLandmarkTypesArray) in UIUtilities.poseConnectionsWarriorU() {
...
//                let startColor = UIColor(Color("Success"))
//                let endColor = UIColor(Color("Success"))
        ...
        for landmarkType in landmarksToDraw() {
...
//        }
    ...
    private static func landmarksToDraw() -> [PoseLandmarkType] {
    ...
    private static func poseConnectionsWarriorU() -> [PoseLandmarkType: [PoseLandmarkType]] {
...
//          PoseLandmarkType.leftIndexFinger: [PoseLandmarkType.leftPinkyFinger],
    ...
    //        to: Float32.self, capacity: Int(maskBytesPerRow * height))
    //
